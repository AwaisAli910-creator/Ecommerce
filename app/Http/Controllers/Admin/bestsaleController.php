...
class bestsaleController extends Controller
{
    public function index(){
        ...
        $bestsales=Bestsale::orderby('id', 'desc')->paginate(10);
        return view('admin.bestsale.index')->with(compact('bestsales'));
  ...
  return view('admin.bestsale.add');
   } 
   ...
   public function store(Request $request)
   {
    ...
    if (request()->hasFile('p_img')) {
        ...
        $file->move(public_path('uploads/bestsale_img'), $filename);
    } else {
 ...
 return redirect()->to(route('bestsale.index'))->with('success', 'Product Added successfully.');
   ...
   public function show(string $id)
    {
        $bestsale = Bestsale::findOrFail($id); // Fetch the product by ID
    return view('admin.product_details', compact('product')); // Pass the product to the view
    }

    public function edit (string $id){
        return view('admin.bestsale.edit');
       ...
       public function destroy(string $id)
    {
        $bestsale = Bestsale::find($id);
        $bestsale->delete();
        return redirect()->to(route('bestsale.index'));
    }
}
